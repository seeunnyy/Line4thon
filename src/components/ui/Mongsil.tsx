...
// motion="lively"(기본)는 고개 갸웃·통통·뒤뚱뒤뚱 걷기에 표정(무표정→웃음→눈 뜸→윙크)까지 바뀌고, "calm"은 원본 표정으로 작게만 움직인다.
// 소품(해·구름·빗방울)은 따로 얹어서 CSS로 움직이고, 몽실이가 걸으면 함께 옆으로 따라간다. 좌표는 세 모습이 함께 쓰는 590×672 캔버스 기준(mongsil/layout.ts)이고,
...
export default function Mongsil({
  ...
  shadow?: boolean;
  // 움직임 크기: "lively" = 크게 + 표정 바뀜(기본), "calm" = 작게 + 원본 표정
  motion?: MotionMode;
  className?: string;
}) {
  const a = animate;
  ...
  return (
    ...
    >
      <MongsilBody weather={weather} animate={animate} shadow={shadow} motion={motion}>
        {weather === "sunny" && (
