...
- `src/`: application source code
  ...
  - `src/components/ui/`: 공통 UI 컴포넌트 (Button, Chip, Card, Header, BottomSheet 등). 아바타는 `Avatar`(원형 얼굴)와 `Mongsil`(전신 + 날씨 소품). 전신의 몸통은 클라이언트 컴포넌트 `MongsilBody`가 정지 그림 위에 2.5D WebGL 캔버스를 얹어 움직이고, 그 엔진은 `src/components/ui/mongsil/`(`layout.ts` 좌표·`motion.ts` 날씨별 안무(calm/lively)·`engine.ts` WebGL(팔·가방 가중치는 로드할 때 몸 그림 색으로 만든 끈·가방 마스크로 정점마다 굽는다)·`faceData.ts` 얼굴 부품 좌표, 외부 라이브러리 없음)에 있다. WebGL을 못 쓰면 정지 그림 + CSS 숨쉬기로 대체된다
